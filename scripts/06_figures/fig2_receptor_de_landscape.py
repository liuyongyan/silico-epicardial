...
Panel A: Volcano Plot - All Receptors
Panel B: Receptor Ranking Bar Plot (Top 20)
...
# Cap logFC for display
rec['logfc_capped'] = np.clip(rec['logfoldchanges'], -15, 15)
...
# ---- Panel B: Top 20 Receptors Bar Plot ----
ax = axes[1]
top20 = rec[rec['logfoldchanges'] > 0].nlargest(20, 'logfoldchanges').iloc[::-1]
colors = [pw_colors.get(top20.iloc[i]['pathway'], '#BDC3C7') for i in range(len(top20))]
bars = ax.barh(range(len(top20)), np.clip(top20['logfoldchanges'], 0, 15), color=colors)

# Mark FGFR2 position
for i, (_, row) in enumerate(top20.iterrows()):
    ax.text(np.clip(row['logfoldchanges'], 0, 15) + 0.2, i,
...
ax.set_xlabel('logFC (capped at 15)', fontsize=10)
ax.set_title('B. Top 20 Upregulated Receptors', fontsize=12, fontweight='bold')
...
for pw in pathways_of_interest:
    ...
    n_up = (sub['logfoldchanges'] > 0).sum()
    n_down = (sub['logfoldchanges'] < 0).sum()
